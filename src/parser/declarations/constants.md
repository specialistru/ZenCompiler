Следующий файл — `parser/declarations/constants.c`.

---

### Задача `constants.c` (declarations):

Парсинг инструкции `CONSTANTS` в ABAP — объявление неизменяемых значений с типами и начальными значениями.

---

## Реализация `constants.c`
...
* Поддерживает стандартный синтаксис:
  ...
  CONSTANTS title TYPE c VALUE 'Manager'.
  ```

* Использует узлы:

  * `AST_CONSTANT_DECL` — основной узел;
  * `AST_TYPE_SPEC` — тип;
  * `AST_VALUE_SPEC` — значение.

* Требует обязательного указания типа и значения — как в ABAP.
