Конечно! Следующий файл — `parser/if/else.c`.

---

### Задача `else.c`:

Реализовать парсер оператора IF с поддержкой ветки ELSE (без ELSEIF):
...
* Парсим IF, тело до ELSE или ENDIF.
* Если есть ELSE, парсим тело ELSE.
* В конце проверяем ENDIF.
* Возвращаем AST с условием, телом и веткой else.

---

Если нужно — продолжу по списку!
