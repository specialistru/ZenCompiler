Держи реализацию для следующего файла — `parser/if/elseif.c`.

---

### Задача `elseif.c`:

Реализовать парсер оператора IF с поддержкой ветвления ELSEIF, то есть конструкций вида:
...
* Условия ELSEIF парсятся так же, как IF.
* Ветка ELSE необязательна.
* Формируем структуру AST, где IF узел содержит список ветвей (условных и else).

---

## Пример реализации `elseif.c`
...
* Обрабатываем IF, затем тело.
* Цикл парсит любые ELSEIF, создавая AST для каждой ветки.
* Опциональная ветка ELSE парсится последней.
* Завершается парсер при ENDIF.

---

Если хочешь, могу реализовать ещё один файл из списка — пиши!
