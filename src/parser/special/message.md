Следующий файл — `parser/special/message.c`.

---

### Задача `message.c` (special):

Обработка оператора `MESSAGE` в ABAP, который выводит системные сообщения (ошибки, предупреждения, информационные).

---

## Реализация `message.c`
